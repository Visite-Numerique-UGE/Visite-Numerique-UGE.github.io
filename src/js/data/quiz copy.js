...
function question(id_parcours, step) {
  ...
  nb.then((nb) => {
    ...
    for (let i = 0; i < nb; i++) {
      ...
      button_progress += `<button ` + disabled + ` id="progress" @click="count=` + i + `">` + (i + 1) + ` </button>\n`
    }


    console.log(button_progress)
    console.log("progress")
    let progress_bar = `<div id="progressbar">
                  ...
                  <div class="answerblock center">
                      ...
                      <template x-for="i in 4">
                        <div class="button-borders">
                          <button
                            ...
                            @click="if(await quizFun.verification(quizFun.randomAnswer(id_parcours, step, (await dataFun.getField(1,id_parcours, step, 3)=='multiple') ? i : -1),  id_parcours, step)) (((await quizFun.andMore(id_parcours, step)) ? count = 0 : count++), location.reload())"
                          ></button>
                        </div>
                      </template>
                    </template>
                    ...
                    <template x-if="await dataFun.getField(1,id_parcours, step, 3)=='saisie'  ">
                      <div x-data="{ res: '' }">
                        ...
                        <br /><br />
                        <button
                          @click="if(await quizFun.verification(res,  id_parcours, step)) (((await quizFun.andMore(id_parcours, step)) ? count = 0 : count++), location.reload())">
                          Valider
                        </button>
                      </div>
                    </template>
                    <template x-if="await dataFun.getField(1,id_parcours, step, 3)=='lieu' || await dataFun.getField(1,id_parcours, step, 3)=='anecdote'">
                      <button @click="(((await quizFun.andMore(id_parcours, step)) ? count = 0 : count++), location.reload())">
                        J'y suis
                      </button>
                    </template>
                  </div>
